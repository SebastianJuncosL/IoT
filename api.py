...
# Poner el account_sid y auth_token aqui
...
class MESSAGE(Resource):
    def post(self):
        ...
        if message_body.find("heart rate") == 0:
            ...
            mensaje = " " + \
                db.find_user(number) + \
                " esta es la información actual de tus registros"
            print(number)
            message = client.messages.create(
                from_='whatsapp:+14155238886',
                body=mensaje,
                media_url=[
                    'https://549dbc8d671e.ngrok.io/image?number='+number],
                to=number
            )
        else:
            try:
                mensaje = "Hola " + \
                    db.find_user(number) + \
                    " puedes pedirme tu Heart Rate por el momento."
                message = client.messages.create(
                    from_='whatsapp:+14155238886',
                    body=mensaje,
                    to=number
